class Sorcistino extends Esc {
    ...
    render(startingScreen=0) {
        ...
        `
        this.setupEvents();
        Esc.dispatchEvent("sorcistino:rendered",this)
    ...
    show(screen) {
        ...
        if (screen<0) {
            this.screen = 0;
            Esc.dispatchEvent("sorcistino:reset",this)
            return;
        }
...
class SorcistinoModal extends Sorcistino {
    constructor(options) {
        super(options)
        this.root = document.querySelector(".esc-modal .modal-text");
    }
}
