class SrDModal extends SrD {
    static show(content,closeable=true) {
        let modal = document.querySelector(".srd-modal");
        if (!modal) {
            modal = document.createElement("DIV");
            modal.classList.add("srd-modal");
        ...
        if (closeable){
          ...
          modal.querySelector(".close").addEventListener("click",event=>{
              ...
              document.dispatchEvent(new CustomEvent("srd:modal:closed",{bubbles:true}))
          });
          modal.addEventListener("click",event=>{
            if (event.target.classList.contains("srd-modal")) {
                modal.classList.remove("active");
                modal.style.top = "0px";
                modal.innerHTML = "";
                document.querySelector("body").classList.remove("stop-scrolling");
