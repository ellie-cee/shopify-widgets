...
class SrDVariableStore {
  static setValues(textObject) {
    
    if (window._SrDVariableStoreValues) {
      window._SrDVariableStoreValues = {...window._SrDVariableStoreValues,...textObject}
    } else {
      window._SrDVariableStoreValues = textObject;  
    }
  }
  static intFor(key,defaultValue) {
    return parseInt(SrDVariableStore.textFor(key,defaultValue))
  }
  static textFor(key,defaultValue) {
    
    if (window._SrDVariableStoreValues[key]) {
      return window._SrDVariableStoreValues[key]
    } else {
      if (defaultValue) {
        return defaultValue;
      } else {
  ...
  async changeCart(payload) {
   ...
   static setupDawnProductForm(form) {
      form.querySelector("button").addEventListener("click",event=>{
         ...
         new SrD().addToDawnCart(form).then(response=>{
    ...
    async addToDawnCart(form) {
       ...
       return this.fetch(this.siteUrl("/cart/add"),{
           ...
           .then(json=>{
             ...
             if (json.status) {
               SrDModal.show(json.description);
             } else {
        ...
        case "SELECT":
            ...
            field.addEventListener("change",event=>{
                if (window._form_montor_on) {
                  ...
                  SrD.dispatchEvent("srd:formValue:changed",{field:argument,value:field.options[field.selectedIndex].value});
                }
          })
          break;
        default:
          ...
          this.observers[argument] = new MutationObserver((list,observer)=>{
              ...
              SrD.dispatchEvent("srd:formValue:changed",{field:argument,value:field.value});  
...
class SrDStorefront extends SrD {
  requires() {
